...
.factory("auth", function($location, login) {
    return{
        login : function(username, password)
        {
            var query = login.get({ id: username, password:  password }, function () {
                if (query.login[0] && query.login[0].profile_email) {
                    ...
                    sessionStorage.account_email = query.login[0].account_email;
        ...
        checkStatus : function()
        {
            ...
            var rutasPrivadas = ["dashboard","profile","password","users","stores","customers","courses","student","teacher","course"];
